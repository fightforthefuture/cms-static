...
$('input').placeholder();var socialSites=['google-plus','facebook','tumblr','twitter'];for(var i in socialSites){var target='.social-'+socialSites[i];$(target).attr('href',$(target).attr('href').replace(/%3Ft%3D[\w%]+/,''));}
var tag=$('input[name=tag]').val()||'untagged';if(tag!=='untagged'&&$('#progress-bar-count').length){var url='https://fftf-cache.herokuapp.com/signatures?tag='+tag;$.getScript(url);window.signaturesCallback=function(res){var count=res.data.count;var numAnim=new countUp("progress-bar-count",1,count,0,2);numAnim.start();$('.progress-bar-signatures').css({opacity:1});var goal;goal=1000;goal=10000;var percent=count/goal;percent=Math.min(percent,1);percent*=100;$('.progress-bar-inner').width(percent+'%');}}
if(qs.t){var data={t:qs.t,page_id:tag,page_type:'Fein'};$.ajax({data:data,dataType:'jsonp',url:'https://platformzz.herokuapp.com/api/movements/1/email_tracking/email_clicked'});$('form').each(function(){var $form=$(this);if(!$form.find('[name=t]').length){var $input=$('<input name="t" type="hidden" />');$input.val(qs.t);$form.append($input);}});}
