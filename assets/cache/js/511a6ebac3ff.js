...
function safeActiveElement(){try{return document.activeElement;}catch(err){}}}(this,document,jQuery));!function($){"use strict";$(function(){$.support.transition=(function(){var transitionEnd=(function(){var el=document.createElement('bootstrap'),transEndEventNames={'WebkitTransition':'webkitTransitionEnd','MozTransition':'transitionend','OTransition':'oTransitionEnd otransitionend','transition':'transitionend'},name
...
$this.wrap('<div class="fluid-width-video-wrapper"></div>').parent('.fluid-width-video-wrapper').css('padding-top',(aspectRatio*100)+"%");$this.removeAttr('height').removeAttr('width');});});};})(jQuery);(function($){var qs=querystring.decode();$(function(){$('body').hide().show();if('salsa_success'in qs){$('[data-action="'+qs['salsa_success']+'"]').modal();showDonateUI();}
...
$buttons.attr('disabled','disabled').text('Working...');$.post($form.attr('action'),$form.serialize(),function(data){$('[data-action="'+id+'"]').modal();showDonateUI();$buttons.text('Thanks!');});e.preventDefault();});});if(window.location.hash.indexOf('#thanks')!==-1){$('.modal:first').modal();showDonateUI();}
...
if(pathname.indexOf("teenager")!==-1&&pathname.indexOf("salsa")!==-1){setTimeout(function(){$.teenage_modal.close();$.modal();showDonateUI();},500)}});})(jQuery);function showDonateUI(){if(!$('#fftf_donate_tag').length)
return console.log('No tag');var tag=$('#fftf_donate_tag').html();var email='';if($('#id_email'))
email=$('#id_email').val();var options={tag:tag,email:email};FFTFDonate.init(options).test($('#fftf_donate')[0],'stripe10_vs_external',[{name:'stripe10',ui:'button',buttonText:'Donate $10',amount:10,checkoutFlow:'stripe'},{name:'external',ui:'button',buttonText:'Donate'}]);}
