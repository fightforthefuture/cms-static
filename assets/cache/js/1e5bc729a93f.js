...
$('input').placeholder();var tag=$('input[name=tag]').val()||'untagged';if(tag!=='untagged'&&$('#progress-bar-count').length){var url='https://fftf-cache.herokuapp.com/signatures?tag='+tag;$.getScript(url);console.log(url);window.signaturesCallback=function(res){var count=res.data.count;var numAnim=new countUp("progress-bar-count",1,count,0,2);numAnim.start();$('.progress-bar-signatures').css({opacity:1});var goal;goal=1000;goal=10000;var percent=count/goal;percent=Math.min(percent,1);percent*=100;$('.progress-bar-inner').width(percent+'%');}}
