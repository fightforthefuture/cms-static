...
$('input').placeholder();if(qs.t&&qs.t.match(/^[0-9A-z]+$/)){var tag=$('input[name=tag]').val();$.ajax({data:{t:qs.t,page_id:tag,page_type:'Fein'},type:'POST',url:'https://platformzz.herokuapp.com/api/movements/1/email_tracking/email_clicked'});$('form').each(function(){var $form=$(this);if(!$form.find('[name=t]').length){var $input=$('<input />');$input.attr('name','t');$input.attr('type','hidden');$input.val(qs.t);$form.append($input);}});}
var $actions=$('.action.salsa');$actions.each(function(){var $action=$(this),id=$action.find('[name="action"]').val();$action.find('form').bind('submit',function(e){e.preventDefault();var $form=$(this),$buttons=$form.find('button');$buttons.attr('disabled','disabled').text('Working...');$.post($form.attr('action'),$form.serialize(),function(data){$('[data-action="'+id+'"]').modal();$buttons.text('Thanks!');});});});if(window.location.hash.indexOf('#thanks')!==-1){$('.modal:first').modal();}
