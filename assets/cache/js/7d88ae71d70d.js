...
$('input').placeholder();if(qs.t){var tag=$('input[name=tag]').val()||'untagged',data={t:qs.t,page_id:tag,page_type:'Fein'};$.ajax({data:data,dataType:'jsonp',url:'https://platformzz.herokuapp.com/api/movements/1/email_tracking/email_clicked'});$('form').each(function(){var $form=$(this);if(!$form.find('[name=t]').length){var $input=$('<input name="t" type="hidden" />');$input.val(qs.t);$form.append($input);}});}
