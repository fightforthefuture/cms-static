...
var $actions=$('.action.salsa');$actions.each(function(index,action){var $action=$(action),id=$action.find('[name="action"]').val();$action.find('form').bind('submit',function(evt){evt.preventDefault();var $form=$(this),$buttons=$form.find('button');$buttons.attr('disabled','disabled').text('Working...');$.post($form.attr('action'),$form.serialize(),function(data){$('[data-action="'+id+'"]').modal();$buttons.text('Thanks!');});});});if(window.location.hash=='#thanks'){$('.modal:first').modal();}
